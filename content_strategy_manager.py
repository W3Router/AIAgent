...
"""
    ...
    def evaluate_content(self, content, platform='twitter'):
        ...
        length_margin = 20  # Allow 20 characters margin
        meets_length = (length_rules['min'] - length_margin) <= content_length <= (length_rules['max'] + length_margin)
        is_optimal_length = abs(content_length - length_rules['optimal']) <= 40  # Allow 40 characters deviation from optimal
        
        evaluation = {
            'meets_length': meets_length,
            'is_optimal_length': is_optimal_length,
            'has_call_to_action': any(cta.lower() in content.lower() 
                                    for cta in self.strategy['content_guidelines']['call_to_action']['types']),
        ...
        print(f"Length rules: min={length_rules['min']}, max={length_rules['max']}, optimal={length_rules['optimal']}")
